...
CREATE TABLE public.childcare_checkins (
  ...
  checked_in_by UUID NOT NULL, -- staff member who checked in
  checked_out_by UUID, -- staff member who checked out
  drop_off_notes TEXT,
  pick_up_notes TEXT,
  child_condition_checkin TEXT DEFAULT 'good',
  child_condition_checkout TEXT,
  authorized_pickup_person TEXT,
  pickup_person_id TEXT, -- ID verification
...
CREATE TABLE public.childcare_daily_reports (
  ...
  meals JSON DEFAULT '[]'::json, -- [{meal: 'breakfast', time: '09:00', notes: 'ate well'}]
  naps JSON DEFAULT '[]'::json, -- [{start: '13:00', end: '14:30', notes: 'slept well'}]
  diaper_changes JSON DEFAULT '[]'::json, -- [{time: '10:30', type: 'wet', notes: ''}]
  activities JSON DEFAULT '[]'::json, -- [{activity: 'story time', time: '11:00', participation: 'active'}]
  mood_observations TEXT,
  learning_milestones TEXT,
  social_interactions TEXT,
  parent_communication TEXT,
  photos_taken INTEGER DEFAULT 0,
  incidents JSON DEFAULT '[]'::json, -- [{time: '15:00', type: 'minor', description: '', action_taken: ''}]
...
CREATE INDEX idx_child_profiles_age ON public.child_profiles(age_months);
...
CREATE INDEX idx_childcare_checkins_organization_date ON public.childcare_checkins(organization_id, check_in_time::date);
...
-- Function to auto-update current children count
CREATE OR REPLACE FUNCTION update_childcare_staff_capacity()
RETURNS TRIGGER AS $$
BEGIN
  -- Update staff capacity when children check in/out
  IF TG_OP = 'INSERT' AND NEW.status = 'checked_in' THEN
    UPDATE childcare_staff_schedules 
    SET current_children_count = current_children_count + 1
    WHERE staff_id = NEW.checked_in_by 
    AND schedule_date = NEW.check_in_time::date
    AND NEW.check_in_time::time BETWEEN start_time AND end_time;
  END IF;
  
  IF TG_OP = 'UPDATE' AND OLD.status = 'checked_in' AND NEW.status = 'checked_out' THEN
    UPDATE childcare_staff_schedules 
    SET current_children_count = GREATEST(0, current_children_count - 1)
    WHERE staff_id = NEW.checked_out_by 
    AND schedule_date = NEW.check_out_time::date
    AND NEW.check_out_time::time BETWEEN start_time AND end_time;
  END IF;
